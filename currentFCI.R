...
# 100 parallel computations
registerDoParallel(100)


# a function that performs bootstrapping for currentFCI

bootstrap <- function(data, nBoot, pBoot, alpha, test, 
                      partners, posOut, posMisc,
                      doPdsep, conservative, m.max, pdsep.max,
                      posBio = NULL, correlation = "pearson", filename = 'models/model/',
                      indepTest = gaussCItest) {
  
  # generate a list of all nodes (-1 to exclude blocks variable)
  nodes <- colnames(data)[1:(ncol(data) - 1)]
  
  # generate matrices for the different potential edge mark types
  # 1: circle, 2: arrowhead, 3: tail
  Mat1 <- matrix(ncol = ncol(data) - 1, nrow = ncol(data) - 1, 0)
  rownames(Mat1) <- nodes
  colnames(Mat1) <- nodes
  
  Mat2 <- matrix(ncol = ncol(data) - 1, nrow = ncol(data) - 1, 0)
  rownames(Mat2) <- nodes
  colnames(Mat2) <- nodes
  
  Mat3 <- matrix(ncol = ncol(data) - 1, nrow = ncol(data) - 1, 0)
  rownames(Mat3) <- nodes
  colnames(Mat3) <- nodes
  ...
  res <- foreach(run = 1:nBoot) %dopar% {
    ...
    # add random blocks to dataset until requested size is reached
    while (nrow(dataSet) < round(pBoot*nrow(data))/2) {
        samp <- sample(x = unique(data$block), size = 1)
        dataSet <- rbind(dataSet, data[data$block == samp,])
    }
    ...
    dataSet <- list(C = cor(dataSet, method = correlation), n = nrow(dataSet), dat = dataSet, test = test)
    print('a')    
    # apply the algorithm
    result <- currentFci(dataSet, indepTest, alpha, rownames(dataSet$C), partners = partners, posOut = posOut, posRem = posMisc, doPdsep = doPdsep, conservative = conservative, m.max = m.max, pdsep.max = pdsep.max, posBio = posBio)

    # get the causal matrix
    mat <- attributes(result)$amat
    
    # get edge mark matrices
    Mat1 <- as.numeric(mat == 1)
    Mat2 <- as.numeric(mat == 2)
...
# a function modified to perform different conditional independence tests
gaussCItest <- function(x,y,S,suffStat) {
  ## suffStat$C: correlation matrix
  ## suffStat$n: sample size
  
  # randomized conditional independence test (relatively fast, but not very accurate)
  if (suffStat$test == 'RCoT') {
    datx <- suffStat$dat[,x]
    daty <- suffStat$dat[,y]
    if(length(S) == 0) {
      datS <- NULL
    ...
    res <- RCoT(datx, daty, datS)$p
    return(res)
    
  # correlation based independence test
  } else if (suffStat$test == 'gauss') {
    z <- zStat(x,y,S, C = suffStat$C, n = suffStat$n)
    return(2*pnorm(abs(z), lower.tail = FALSE))
    
  # kernel based conditional independence test (slow, but more accurate)
  } else if (suffStat$test == 'KCIT') {
    datx <- suffStat$dat[,x]
    daty <- suffStat$dat[,y]
    if(length(S) == 0) {
      datS <- NULL
